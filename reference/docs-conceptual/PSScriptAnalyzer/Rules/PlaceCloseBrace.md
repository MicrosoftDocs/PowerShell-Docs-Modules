...
This rule detects close braces that aren't placed on a new line by themselves or that are followed
by empty lines. Close brace placement should follow a consistent style. It should be on a new line
by itself and shouldn't be followed by an empty line. This rule is **disabled** by default.
...
brace. It accepts a boolean value. To enable this check, set this parameter to `$true`. The
...
accepts a boolean value. To disable this rule, set this parameter to `$false`. The default value is
`$true`.
